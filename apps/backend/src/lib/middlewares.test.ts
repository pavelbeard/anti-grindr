...
describe("middlewares", () => {
  ...
  describe("validate", () => {
    ...
    it("should throw an error if validation fails.", async () => {
      ...
      expect(next).toHaveBeenCalledWith(
        new AppError("validation", "Invalid or missing inputs provided for: email, password")
      );
      expect(next.mock.calls[0][0]).toBeInstanceOf(AppError);
      expect(next.mock.calls[0][0].message).toBe("Invalid or missing inputs provided for: email, password");
